		       ...
		       struct archivo_csv *archivo, size_t columnas)
{
	...
	while (i < columnas && !error) {
		...
		if (archivo->string[i] == NULL) {
			...
			escribir_subcadena(string, long_subcadena, letra_actual, i, archivo);
			i++;
		}
...
struct archivo_csv *dividir_string(const char *string, char separador, size_t columnas, struct archivo_csv* archivo)
{
    char **str_temp = realloc(archivo->string, sizeof(char *) * columnas);
    if (str_temp == NULL) {
        liberar_memoria_en_substr(archivo, columnas);
        return NULL;
    }
    archivo->string = str_temp;
    
    escribir_palabras(string, separador, archivo, columnas);
    return archivo;
}

void liberar_partes(struct archivo_csv *partes, size_t cant_asignada)
{
	if (partes != NULL){
		for (size_t i = 0; i < cant_asignada; i++) {
...
size_t leer_linea_csv(struct archivo_csv *archivo, size_t columnas, bool (*funciones[])(const char *, void *), void *ctx[]){
	char texto[200];
	if(fgets(texto,sizeof(texto),archivo->nombre_archivo)==0){
		return 0;
	}
	struct archivo_csv *resultado = dividir_string(texto,archivo->separador,columnas,archivo);
	if (resultado == NULL){
		return 0;
	}	
	liberar_partes(resultado,columnas);
	return 2;
}
...
// 		size_t columnas = 5;
//         struct archivo_csv *resultado = dividir_string(texto, archivo->separador, columnas, &partes);
//         if (resultado != NULL) {
