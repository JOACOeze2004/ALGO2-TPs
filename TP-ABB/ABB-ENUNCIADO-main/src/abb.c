				   ...
				   void *ctx,bool *continua_iteracion)
{
	...
	cantidad += contar_iteraciones_preorden(nodo->izq, f, ctx,continua_iteracion);
	cantidad += contar_iteraciones_preorden(nodo->der, f, ctx,continua_iteracion);
	return cantidad;
}
...
size_t abb_iterar_preorden(abb_t *abb, bool (*f)(void *, void *), void *ctx)
{
	...
	bool continuar_iteracion = true;
	return contar_iteraciones_preorden(abb->raiz, f, ctx, &continuar_iteracion);
}
				    ...
				    void *ctx, bool *continua_iteracion)
{
	...
	size_t cantidad = 0;
	cantidad += contar_iteraciones_postorden(nodo->izq, f, ctx,continua_iteracion);
	cantidad += contar_iteraciones_postorden(nodo->der, f, ctx,continua_iteracion);
	if (*continua_iteracion) {
		if (!f(nodo->elemento, ctx)) {
			*continua_iteracion = false;
			return cantidad;
...
size_t abb_iterar_postorden(abb_t *abb, bool (*f)(void *, void *), void *ctx)
{
	...
	bool continuar_iteracion = true;
	return contar_iteraciones_postorden(abb->raiz, f, ctx, &continuar_iteracion);
}
