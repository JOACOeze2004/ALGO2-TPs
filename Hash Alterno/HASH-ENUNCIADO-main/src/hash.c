...
//post:	Devuelve true si se pudo insertar el nuevo par, independientemente del caso en el que caiga.
...
//post:	Libera lo que habia en la tabla vieja, es decir libera la clave del nodo y el mismo nodo.
...
//pre:	Los parametros pasados no pueden ser null y/o deben haber sido inicializados.
//post:	Va reinsertando los elementos que habia en la tabla vieja pero en la nueva.
...
bool hash_insertar(hash_t *hash, char *clave, void *valor, void **encontrado)
{
	...
	if ((float)hash->cantidad / (float)hash->capacidad >=
	    FACTOR_CARGA_MAX || hash->contador_pares[indice_hash] > LIMITE_PARES) {
...
//post:	Si encontramos la clave devolvemos el elemento, en caso de no encontrarla, devolvemos NULL.
...
//post:	Devolvemos el valor que tenia esa clave que queriamos eliminar, en caso de existir obvio, caso de que no existiera la clave a eliminar, devolvemos NULL.
...
//post:	Vamos liberando los pares (aplicandoles o no el destructor), y luego liberamos el vector de pares y el struct del hash.
