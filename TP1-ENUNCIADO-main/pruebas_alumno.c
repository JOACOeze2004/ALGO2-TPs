...
void crearUnaPokedexResultaEnUnaPokedexVacia() {
    ...
    pa2m_afirmar(pokedex != NULL, "Crear una pokedex resulta en una pokedex no nula");
    pa2m_afirmar(pokedex_cantidad_pokemones(pokedex) == 0, "Una pokedex recién creada está vacía");
...
void agregarUnPokemonALaPokedexAumentaSuCantidad() {
    ...
    pa2m_afirmar(pokedex_agregar_pokemon(pokedex, pikachu), "Se puede agregar un pokemon a la pokedex");
    pa2m_afirmar(pokedex_cantidad_pokemones(pokedex) == 1, "La cantidad de pokemones en la pokedex es 1 después de agregar un pokemon");
...
void agregarUnPokemonALaPokedexRepetidoNoTeDeja() {
    ...
    pa2m_afirmar(pokedex_agregar_pokemon(pokedex, pikachu), "Se puede agregar un pokemon a la pokedex");
    pa2m_afirmar(pokedex_cantidad_pokemones(pokedex) == 1, "La cantidad de pokemones en la pokedex es 1 después de agregar un pokemon");
    pa2m_afirmar(!pokedex_agregar_pokemon(pokedex, pikachu), "No se puede agregar un pokemon que a estaba en la pokedex");
    pa2m_afirmar(pokedex_cantidad_pokemones(pokedex) == 1, "La cantidad de pokemones en la pokedex se mantiene en 1 después deintentar agregar un pokemon");
...
void buscarUnPokemonEnLaPokedexDevuelveElPokemonCorrecto() {
    ...
    const struct pokemon* encontrado = pokedex_buscar_pokemon(pokedex, "Charmander");
    pa2m_afirmar(encontrado != NULL, "Buscar un pokemon existente devuelve un puntero no nulo");
    pa2m_afirmar(strcmp(encontrado->nombre, "Charmander") == 0, "El nombre del pokemon encontrado es 'Charmander'");
...
void buscarUnPokemonInexistenteEnLaPokedexDevuelveNull() {
    struct pokedex* pokedex = pokedex_crear();
    const struct pokemon* encontrado = pokedex_buscar_pokemon(pokedex, "Bulbasaur");

    pa2m_afirmar(encontrado == NULL, "Buscar un pokemon inexistente devuelve NULL");
...
void buscarUnPokemonInexistenteEnUnaPokedexLlenaDevuelveNull() {
    ...
    struct pokemon charmander = {"Charmander", TIPO_FUEGO, 50, 2, 24};      // la primer aletra del primer pokemon deberia ser mayuscula
    ...
    // Intentar buscar un Pokémon que no existe en la pokédex
    const struct pokemon* encontrado = pokedex_buscar_pokemon(pokedex, "Mewtwo");

    if (encontrado == NULL) {
        printf("\nPokemon no encontrado en la Pokedex llena, devuelve NULL\n");
    } else {
        printf("\nError: Se encontró un pokemon cuando no debería en la Pokedex llena\n");
    }

    // Afirmación para verificar el comportamiento esperado
    pa2m_afirmar(encontrado == NULL, "Buscar un pokemon inexistente en una Pokedex llena devuelve NULL");
...
void iterarSobreLosPokemonesDeLaPokedexInvocaLaFuncionCorrectamente() {
    ...
    size_t iterados = pokedex_iterar_pokemones(pokedex, mostrarPokemon, NULL);
    pa2m_afirmar(iterados == 2, "Se iteró sobre los dos pokemones agregados");

    pokedex_destruir(pokedex);
}

void iterarSobreTodosLosPokemonesDeLaPokedexInvocaLaFuncionCorrectamente2() {
    struct pokedex* pokedex = pokedex_crear();
    ...
    size_t iterados = pokedex_iterar_pokemones(pokedex, mostrarPokemon, NULL);
    pa2m_afirmar(iterados == 9, "Se iteró sobre los nueve pokemones agregados");

    // Opcionalmente, podrías hacer una afirmación específica por cada Pokémon
    const struct pokemon* encontrado = pokedex_buscar_pokemon(pokedex, "Pikachu");
    pa2m_afirmar(encontrado != NULL && strcmp(encontrado->nombre, "Pikachu") == 0, "Pikachu está en la pokedex");

    encontrado = pokedex_buscar_pokemon(pokedex, "Charmander");
    pa2m_afirmar(encontrado != NULL && strcmp(encontrado->nombre, "Charmander") == 0, "Charmander está en la pokedex");

    encontrado = pokedex_buscar_pokemon(pokedex, "Larvitar");
    pa2m_afirmar(encontrado != NULL && strcmp(encontrado->nombre, "Larvitar") == 0, "Larvitar está en la pokedex");

    encontrado = pokedex_buscar_pokemon(pokedex, "Cacnea");
    pa2m_afirmar(encontrado != NULL && strcmp(encontrado->nombre, "Cacnea") == 0, "Cacnea está en la pokedex");

    encontrado = pokedex_buscar_pokemon(pokedex, "Joltik");
    pa2m_afirmar(encontrado != NULL && strcmp(encontrado->nombre, "Joltik") == 0, "Joltik está en la pokedex");

    encontrado = pokedex_buscar_pokemon(pokedex, "Togepi");
    pa2m_afirmar(encontrado != NULL && strcmp(encontrado->nombre, "Togepi") == 0, "Togepi está en la pokedex");

    encontrado = pokedex_buscar_pokemon(pokedex, "Floatzel");
    pa2m_afirmar(encontrado != NULL && strcmp(encontrado->nombre, "Floatzel") == 0, "Floatzel está en la pokedex");

    encontrado = pokedex_buscar_pokemon(pokedex, "Dedenne");
    pa2m_afirmar(encontrado != NULL && strcmp(encontrado->nombre, "Dedenne") == 0, "Dedenne está en la pokedex");

    encontrado = pokedex_buscar_pokemon(pokedex, "Magcargo");
    pa2m_afirmar(encontrado != NULL && strcmp(encontrado->nombre, "Magcargo") == 0, "Magcargo está en la pokedex");
...
int main()
{
    ...
    iterarSobreTodosLosPokemonesDeLaPokedexInvocaLaFuncionCorrectamente2();
