...
//pre: asumimos que los paramatros son validos.
//post:	Devuelve true si hay un null dentro del vector de funciones sino devuelve false (sirve para evitar perdidas de memoria en caso de que haya un null, perdia memoria del malloc hecho para parsear strings).
...
int main(int argc, char const *argv[])
{
	...
	while (leer_linea_csv(archivo, columnas, funciones, ctx) == 5) {
		...
		if (!pokedex_agregar_pokemon(nueva_pokedex, pokemon) ||
		    son_funciones_invalidas(columnas, funciones)) {
