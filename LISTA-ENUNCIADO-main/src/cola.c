...
{
    if (cola == NULL || cola->lista->nodo_inicio == NULL || cola->lista->cantidad == 0)
    {
        return NULL;
    }    
...
void *cola_desencolar(Cola *cola)
{
    ...
    if (lista_quitar_elemento(cola->lista,0,&elemento_quitado))  //tecnicamente es O(n) pero el pero caso nunca se daria x lo que seria O(1)
    {
