...
void LaCantidadDeElementosInicialesEnListaNoNulaEs0()
{
	...
	pa2m_afirmar(cantidad_de_elementos == 0,
...
void AgregarMasAllaDeCantidadDevuelveFalse()
{
	...
	pa2m_afirmar(
		!se_agrega,
		"No puedo agregar mas alla de la cantidad de elementos que haya");
	pa2m_afirmar(lista_cantidad_elementos(lista) == 0,
		     "La cantidad inicial de una lista se mantiene en 0");
	bool se_agrega_redux = lista_agregar_elemento(lista, 0, NULL);
	pa2m_afirmar(se_agrega_redux, "Puedo agregar si el cosa* es NULL");
...
void AgregarCorrectamenteAlgoDevuelveTrue()
{
	...
	pa2m_afirmar(
		!lista_obtener_elemento(lista, 0, (void **)&elemento_guardado),
		"No se puede obtener el elemento buscado de una lista vacia");
	void *elemento = (int *)4;
	bool se_agrega = lista_agregar_elemento(lista, 0, elemento);
	pa2m_afirmar(se_agrega, "Se puede agregar un primer elemento");

	pa2m_afirmar(lista_cantidad_elementos(lista) == 1,
		     "La cantidad de una lista se aumenta a 1");

	void *elemento2 = (int *)12;
	bool se_agrega_segundo = lista_agregar_elemento(lista, 1, elemento2);
	pa2m_afirmar(se_agrega_segundo,
		     "Se puede agregar un segundo elemento (al final)");

	pa2m_afirmar(lista_cantidad_elementos(lista) == 2,
		     "La cantidad de una lista se aumenta a 2");

	void *elemento2_de_nuevo = (int *)20;
	bool se_agrega_segundo_de_nuevo =
		lista_agregar_elemento(lista, 2, elemento2_de_nuevo);
	pa2m_afirmar(se_agrega_segundo_de_nuevo,
		     "Se puede agregar un tercer elemento (al final)");

	pa2m_afirmar(lista_cantidad_elementos(lista) == 3,
		     "La cantidad de una lista se aumenta a 3");
...
void AgregarAlFinalAlgoDevuelveTrue()
{
	...
	pa2m_afirmar(se_agrega, "Se puede agregar un primer elemento");
	pa2m_afirmar(lista_cantidad_elementos(lista) == 1,
		     "La cantidad inicial de una lista se aumenta a 1");
	...
	pa2m_afirmar(se_agrega_segundo_de_nuevo,
		     "Se puede agregar un segundo elemento");
	pa2m_afirmar(lista_cantidad_elementos(lista) == 3,
		     "La cantidad de una lista se aumenta a 3");
	lista_destruir(lista);
}
...
void AgregarAlMedioAlgoDevuelveTrue()
{
	...
	bool se_agrega_intermedio1 =
		lista_agregar_elemento(lista, 1, elemento_intermedio1);
	pa2m_afirmar(se_agrega_intermedio1,
		     "Se puede agregar un cuarto elemento");
	pa2m_afirmar(
		lista_cantidad_elementos(lista) == 4,
		"La cantidad de una lista se aumenta a 4 si agrego un elemento de por medio");
	void *elemento_intermedio2 = (int *)10;
	bool se_agrega_intermedio2 =
		lista_agregar_elemento(lista, 1, elemento_intermedio2);
	pa2m_afirmar(se_agrega_intermedio2,
		     "Se puede agregar un quinto elemento");
	pa2m_afirmar(
		lista_cantidad_elementos(lista) == 5,
		"La cantidad de una lista se aumenta a 5 si agrego un elemento en la posicion recientemente agregada");
...
void AgregarAlfinalNULLDevuelveFalse()
{
	...
	pa2m_afirmar(!se_agrega_final,
		     "No se puede agregar un elemento null al final");
	pa2m_afirmar(
		lista_cantidad_elementos(lista) == 2,
		"La cantidad de una lista se mantiene en 2 si intento agregar un elemento nulo al final");
...
void BuscarPokemonDevuelveElPokemon()
{
	...
	void *resultado = lista_buscar_elemento(lista, &charizard,
						comparar_nombre_pokemon);

	pa2m_afirmar(resultado != NULL,
		     "El Pokémon buscado se encuentra en la lista");
	pa2m_afirmar(strcmp(((struct pokemon *)resultado)->nombre,
			    "Charizard") == 0,
		     "El Pokémon encontrado es Charizard");
...
void BuscarPokemonInexistenteDevuelveNull()
{
	...
	void *resultado = lista_buscar_elemento(lista, &pokemon_inexistente,
						comparar_nombre_pokemon);
	pa2m_afirmar(resultado == NULL,
		     "No se encuentra a Ho-Oh porque no existente en la lista");
...
void BuscarPokemonConFuncionNulaDevuelveNull()
{
	...
	pa2m_afirmar(
		resultado == NULL,
		"No se encuentra a Charizard porque la funcion pasada es null");
...
void BuscarPokemonEnListaVaciaDevuelveNull()
{
	...
	void *resultado =
		lista_buscar_elemento(lista, &pikachu, comparar_nombre_pokemon);
	pa2m_afirmar(resultado == NULL,
		     "No se encuentra a Pikachu porque la lista es vacia");
...
void EliminarPosicionMasAllaDeCantidadDevuelveFalse()
{
	...
	pa2m_afirmar(
		!lista_quitar_elemento(lista, 5, (void **)&elemento_guardado),
		"No se puede eliminar posicion 5 porque no existente elemento en la lista mas alla");
...
void EliminarConNingunElementoDevuelveFalse()
{
	...
	char *elemento_guardado = charizard.nombre;
	pa2m_afirmar(
		!lista_quitar_elemento(lista, 0, (void **)&elemento_guardado),
		"No se puede eliminar porque no existente elemento alguno");
	lista_destruir(lista);
}
...
void EliminarPokemonRetoraTrueYCantidadValida()
{
	...
	pa2m_afirmar(lista_cantidad_elementos(lista) == 4,
		     "La cantidad de una lista es 4 ");
	pa2m_afirmar(lista_quitar_elemento(lista, 0,
					   (void **)&elemento_guardado),
		     "Se puede eliminar posicion 0");
	pa2m_afirmar(
		lista_cantidad_elementos(lista) == 3,
		"La cantidad de una lista se reduce a 3 si elimino un pokemon");
	pa2m_afirmar(
		elemento_guardado->fuerza == 55,
		"El elemento se guardo exitosamente (la fuerza es igual a la del elemento borrado)");
...
void EliminarUltimoPokemonRetoraTrueYCantidadValida()
{
	...
	pa2m_afirmar(lista_cantidad_elementos(lista) == 4,
		     "La cantidad de una lista es 4 ");
	pa2m_afirmar(lista_quitar_elemento(lista, 3,
					   (void **)&elemento_guardado),
		     "Se puede eliminar posicion 4");
	pa2m_afirmar(
		lista_cantidad_elementos(lista) == 3,
		"La cantidad de una lista se reduce a 3 si elimino un pokemon");
	pa2m_afirmar(
		elemento_guardado->fuerza == 25,
		"El elemento se guardo exitosamente (la fuerza es igual a la del elemento borrado)");
	pa2m_afirmar(
		strcmp(elemento_guardado->nombre, "Cartepie") == 0,
		"El elemento se guardo exitosamente (la fuerza es igual a la del elemento borrado)");
...
void EliminarPokemonConElementoGuardadoNuloDevuelveTruePeroNoGuardaElemento()
{
	...
	pa2m_afirmar(lista_quitar_elemento(lista, 2, elemento_a_guardar),
		     "Se puede eliminar posicion 3");
	pa2m_afirmar(
		lista_cantidad_elementos(lista) == 3,
		"La cantidad de una lista se reduce a 3 si elimino un pokemon");
	pa2m_afirmar(elemento_a_guardar == NULL,
		     "El elemento no se guardo porque este es null");
...
void EliminarUltimoPokemonDevuelveTrue()
{
	...
	pa2m_afirmar(lista_quitar_elemento(lista, 0,
					   (void **)&elemento_guardado),
		     "Se puede eliminar posicion 2");
	pa2m_afirmar(lista_cantidad_elementos(lista) == 0,
		     "La cantidad de la lista es 0 (queda vacia)");
	pa2m_afirmar(elemento_guardado != NULL,
		     "El elemento se guardo exitosamente");
	lista_destruir(lista);
}
...
void ObtenerElementoEnPosicionInvalidaDevuelveFalse()
{
	...
	char *elemento_guardado = pikachu.nombre;
	pa2m_afirmar(
		!lista_obtener_elemento(lista, 5, (void **)&elemento_guardado),
		"no se puede obtener el elemento porque la cantidad de elementos es menor al elmento buscado");
	pa2m_afirmar(elemento_guardado == pikachu.nombre,
		     "El elemento no se guardo o modifica");
	lista_destruir(lista);
}
...
void ObtenerElementoEnListaVaciaDevuelveFalse()
{
	...
	pa2m_afirmar(
		!lista_obtener_elemento(lista, 0, &elemento_encontrado),
		"Intentar obtener un elemento de una lista vacía devuelve false");
...
void ObtenerElementoConPunteroNuloNoModificaNada()
{
	...
	pa2m_afirmar(
		lista_obtener_elemento(lista, 0, NULL),
		"Obtener un elemento con puntero nulo devuelve true, pero no lo guarda");
...
void ObtenerElementoConElementoBuscadoValidoDevuelveTrue()
{
	...
	pa2m_afirmar(lista_obtener_elemento(lista, 0,
					    (void **)&elemento_guardado),
		     "Se puede obtener el elemento buscado");
	pa2m_afirmar(elemento_guardado != NULL,
		     "El elemento se guardó exitosamente");
	pa2m_afirmar(
		strcmp(elemento_guardado->nombre, "Pikachu") == 0,
		"Se obtiene correctamente el primer nombre del primer Pokémon");
	pa2m_afirmar(lista_obtener_elemento(lista, 0,
					    (void **)&elemento_guardado) &&
			     elemento_guardado->tipo == 'E',
		     "Se obtiene correctamente el tipo del primer Pokémon");
	pa2m_afirmar(lista_obtener_elemento(lista, 0,
					    (void **)&elemento_guardado) &&
			     elemento_guardado->fuerza == 55,
		     "Se obtiene correctamente la fuerza del primer Pokémon");
	pa2m_afirmar(lista_obtener_elemento(lista, 0,
					    (void **)&elemento_guardado) &&
			     elemento_guardado->destreza == 40,
		     "Se obtiene correctamente la destreza del primer Pokémon");
	pa2m_afirmar(
		lista_obtener_elemento(lista, 0, (void **)&elemento_guardado) &&
			elemento_guardado->resistencia == 50,
		"Se obtiene correctamente la resistencia del primer Pokémon");
...
void IterarSinElementosDevuelve0()
{
	...
	size_t elementos_iterados =
		lista_iterar_elementos(lista, continuar_iteracion, NULL);

	pa2m_afirmar(elementos_iterados == 0,
		     "Sin elementos/pokemones en la lista, no se itera");
...
void Iterar3PokemonesDevuelve3ElementosIterados()
{
	...
	pa2m_afirmar(elementos_iterados == 3,
		     "Se iteran correctamente los 3 Pokémon de la lista");
...
void IntentraCrearIteradorExternoConListaNULLDevuelveNULL()
{
	...
	pa2m_afirmar(iterador_externo == NULL,
		     "No se crea el iterador externo si la lista es NULL");
...
void CrearIteradorExternoCorrectamenteNoDevuelveNULL()
{
	...
	Lista_iterador *iterador_externo = lista_iterador_crear(lista);
	pa2m_afirmar(iterador_externo != NULL,
		     "Se crea el iterador externo correctamente");
...
void IteradorDevuelveFalseSiNoHaySiguienteEnListaCon1Elemento()
{
	...
	pa2m_afirmar(
		lista_iterador_hay_siguiente(iterador_externo),
		"El iterador indica correctamente que debo avanzar a la posicion 0 en una lista con 1 elemento");
...
void IteradorAvanzaEnListaLlenaHAstaElFinal()
{
	...
	pa2m_afirmar(
		lista_iterador_hay_siguiente(iterador_externo),
		"El iterador indica correctamente que hay un siguiente elemento en una lista con 4 elemento (si avance a la segunda posicion)");
	lista_iterador_avanzar(iterador_externo);
	pa2m_afirmar(
		lista_iterador_hay_siguiente(iterador_externo),
		"El iterador indica correctamente que hay un siguiente elemento al llegar al anteultimo elemento");
	lista_iterador_avanzar(iterador_externo);
	pa2m_afirmar(
		lista_iterador_hay_siguiente(iterador_externo),
		"El iterador indica correctamente que no hay un siguiente elemento al llegar al ultimo elemento (porque seria el final)");
...
void AvanzarHastaCharizardYPidoEseElementooDevuelveCharizard()
{
	...
	struct pokemon *elemento_actual =
		(struct pokemon *)lista_iterador_obtener_elemento_actual(
			iterador);

	pa2m_afirmar(elemento_actual != NULL,
		     "El iterador no es NULL en una lista completa");
	pa2m_afirmar(
		strcmp(elemento_actual->nombre, "Charizard") == 0,
		"El iterador devuelve correctamente el elemento Charizard");

	lista_iterador_avanzar(iterador);
	struct pokemon *elemento_actual_nuevo =
		(struct pokemon *)lista_iterador_obtener_elemento_actual(
			iterador);

	pa2m_afirmar(
		strcmp(elemento_actual_nuevo->nombre, "Venusaur") == 0,
		"El iterador devuelve correctamente el elemento Venusaur si volvi a avanzar");
	pa2m_afirmar(
		lista_iterador_hay_siguiente(iterador),
		"Hay siguiente antes de llegar al ultimo elemento de la lista");
	lista_iterador_hay_siguiente(iterador),
		"No hay siguiente si llegue al ultimo elemento de la lista";
...
void IntentarAvanzarYPidoElementooDevuelveNULL()
{
	...
	pa2m_afirmar(elemento_actual == NULL,
		     "El iterador es NULL en una lista vacia");
...
void CrearColaRetornaAlgoNoNULL()
{
	...
	pa2m_afirmar(
		cola_esta_vacía(cola),
		"La cantidad de elementos en la cola es 0 si esta esta vacia");
	cola_destruir(cola);
}
void CrearColaVaciaRetornaFrenteNULL()
{
	...
	pa2m_afirmar(cola_frente(cola) == NULL,
		     "El frente de la cola es NULL si esta esta vacia");
	cola_destruir(cola);
}
...
void EncolarCorrectamenteDevuelveTrue()
{
	...
	pa2m_afirmar(cola_encolar(cola, &pikachu),
		     "Se puedo encolar a Picachu");
	pa2m_afirmar(
		cola_cantidad(cola) == 1,
		"La cantidad de elementos en la cola es 1 si agrego 1 elemento correctamente");
	pa2m_afirmar(cola_encolar(cola, &charizard),
		     "Se puedo encolar a Charizard");
	pa2m_afirmar(
		cola_cantidad(cola) == 2,
		"La cantidad de elementos en la cola es 2 si agrego 1 elemento correctamente");
...
void EncolarNULLDevuelveFalse()
{
	...
	pa2m_afirmar(!cola_encolar(cola, NULL),
		     "No se puedo encolar con puntero NULL");
	pa2m_afirmar(
		cola_cantidad(cola) == 1,
		"La cantidad de elementos en la cola sigue siendo 1 si intento agregar un elemento NULL");
...
void EncolarElementosRepetidos()
{
	...
	pa2m_afirmar(
		cantidad == 3,
		"La cantidad de elementos en la cola es correcta después de encolar elementos repetidos");
...
void EncolarCorrectamenteDevuelveTrueYFrenteCorrecto()
{
	...
	pa2m_afirmar(
		...
		"El frente de la cola no es NULL después de encolar al menos un elemento (aca fueron dos)");
	pa2m_afirmar(
		primer_elemento == &pikachu,
		"El primer elemento en la cola debe ser Pikachu (si encole primero a Pichachu)");
...
void DesencolarCorrectamenteDevuelveElElementoQuitado()
{
	...
	pa2m_afirmar(
		cantidad_luego_agregar2 == 4,
		"La cantidad de elementos en la cola es 4 si agrego 4 elementos correctamente");
	struct pokemon *elemento_quitado = cola_desencolar(cola);
	pa2m_afirmar(
		elemento_quitado == &pikachu,
		"El elemento quitado es pichachu ya que este fue el primer que fue encolado");
	pa2m_afirmar(
		cola_cantidad(cola) == 3,
		"La cantidad de elementos en la cola es 3 si descole correctamente");
...
void DesencolarUnaColaVaciaRetornaNuLL()
{
	...
	pa2m_afirmar(elemento_quitado == NULL,
		     "Retorna NULL si intento descolar en una cola vacia ");
	pa2m_afirmar(
		cola_esta_vacía(cola),
		"La cantidad de elementos en la cola sigue siendo de 0 si intento descolar una cola vacia");
...
void DesencolarCorrectamenteDevuelveElElementoQuitadoPeroEnColaConUnElemento()
{
	...
	pa2m_afirmar(cantidad_luego_agregar2 == 1,
	...
	pa2m_afirmar(
		cola_esta_vacía(cola),
		"La cantidad de elementos en la cola seria 0 si descolo el uncio elemento en la cola");
...
void DescolarVariasVecesEnListaConUnElementoDevuelveNULL()
{
	...
	pa2m_afirmar(
		elemento_quitado2 == NULL,
		"Devuelve NULL si ya no puedo descolar porque ya no hay elementos");
...
void DescolarYEncolarPoneAlMismoElemento()
{
	...
	pa2m_afirmar(cola_encolar(cola, elemento_quitado),
		     "La cola tiene 1 elemento ahora");

	struct pokemon *primer_elemento_de_nuevo = cola_frente(cola);
	pa2m_afirmar(primer_elemento_de_nuevo == &pikachu,
		     "El primer elemento en la cola vuelve a ser Pikachu");
...
void CrearpilaRetornaAlgoNoNULL()
{
	...
	pa2m_afirmar(
		pila_esta_vacía(pila),
		"La cantidad de elementos en la pila es 0 si esta esta vacia");
	pila_destruir(pila);
}
...
void CrearpilaVaciaRetornaTopeNULL()
{
	...
	pa2m_afirmar(pila_tope(pila) == NULL,
		     "El tope de la pila es NULL si esta esta vacia");
	pila_destruir(pila);
}
...
void ApilarCorrectamenteDevuelveTrue()
{
	...
	pa2m_afirmar(
		...
		"La cantidad de elementos en la pila es 1 si agrego 1 elemento correctamente");
	pa2m_afirmar(pila_apilar(pila, &charizard),
		     "Se puedo apilar a Charizard");
	pa2m_afirmar(
		pila_cantidad(pila) == 2,
		"La cantidad de elementos en la pila es 2 si agrego 1 elemento correctamente");
...
void ApilarNULLDevuelveFalse()
{
	...
	pa2m_afirmar(!pila_apilar(pila, NULL),
		     "No se puedo apilar con puntero NULL");
	pa2m_afirmar(
		pila_cantidad(pila) == 1,
		"La cantidad de elementos en la pila sigue siendo 1 si intento agregar un elemento NULL");
...
void ApilararCorrectamenteDevuelveTrueYTopeCorrecto()
{
	...
	pa2m_afirmar(
		...
		"El tope de la pila no es NULL después de apilar al menos un elemento (aca fueron dos)");
	pa2m_afirmar(
		ultimo_elemento == &charizard,
		"El tope en la pila debe ser Charizard (si apile primero a Pichachu y despues a Charizard)");
...
void DesapilararCorrectamenteDevuelveElElementoQuitado()
{
	...
	pa2m_afirmar(
		pila_cantidad(pila) == 4,
		"La cantidad de elementos en la pila es 4 si agrego 4 elementos correctamente");
	struct pokemon *elemento_quitado = pila_desapilar(pila);
	pa2m_afirmar(
		elemento_quitado == &blastoise,
		"El elemento quitado es Blastoise ya que este fue el ultimo en apilarse");
	pa2m_afirmar(
		pila_cantidad(pila) == 3,
		"La cantidad de elementos en la pila es 3 si desapile correctamente");
...
void DesapilararUnapilaVaciaRetornaNuLL()
{
	...
	pa2m_afirmar(pila_esta_vacía(pila),
		     "La cantidad de elementos en la pila es 0");
	struct pokemon *elemento_quitado = pila_desapilar(pila);
	pa2m_afirmar(elemento_quitado == NULL,
		     "Retorna NULL si intento desapilar en una pila vacia");
	pa2m_afirmar(
		pila_esta_vacía(pila),
		"La cantidad de elementos en la pila sigue siendo de 0 si intento desapilar una pila vacia");
...
void DesapilarCorrectamenteDevuelveElElementoQuitadoPeroEnpilaConUnElemento()
{
	...
	pa2m_afirmar(
		elemento_quitado == &pikachu,
		"El elemento quitado es pichachu ya que este fue el ultimo en ser apilado");
	pa2m_afirmar(
		pila_esta_vacía(pila),
		"La cantidad de elementos en la pila seria 0 si desapilo el uncio elemento en la pila");
...
void DesapilarVariasVecesEnPilaConUnElementoDevuelveNULL()
{
	...
	pa2m_afirmar(
		elemento_quitado == &pikachu,
		"El elemento quitado es pichachu ya que este fue el ultimo (y unico) en epilarse");
	struct pokemon *elemento_quitado2 = pila_desapilar(pila);
	pa2m_afirmar(
		elemento_quitado2 == NULL,
		"Devuelve NULL si ya no puedo desapilar porque ya no hay elementos");
...
void DesapilarYEncolarPoneAlMismoElemento()
{
	...
	pa2m_afirmar(
		primer_elemento == &pikachu,
		"El ultimo elemento en la pila debe ser Pikachu (si apile unicamente a Pichachu)");
	...
	pa2m_afirmar(pila_apilar(pila, elemento_quitado),
		     "La pila tiene 1 elemento ahora");

	struct pokemon *primer_elemento_de_nuevo = pila_tope(pila);
	pa2m_afirmar(primer_elemento_de_nuevo == &pikachu,
		     "El ultimo elemento en la pila vuelve a ser Pikachu");

	pila_destruir(pila);
}

void SimularPruebaDeAgregadoYEliminacionMixta()
{
	Lista *lista = lista_crear();

	struct pokemon pikachu = { "Pikachu", 'E', 55, 40, 50 };
	struct pokemon bulbasaur = { "Bulbasaur", 'P', 49, 49, 45 };
	struct pokemon charmander = { "Charmander", 'F', 52, 43, 39 };

	pa2m_afirmar(lista_agregar_al_final(lista, &pikachu),
		     "Agregar Pikachu al final de la lista");
	pa2m_afirmar(lista_agregar_al_final(lista, &bulbasaur),
		     "Agregar Bulbasaur al final de la lista");
	pa2m_afirmar(lista_agregar_al_final(lista, &charmander),
		     "Agregar Charmander al final de la lista");

	pa2m_afirmar(lista_cantidad_elementos(lista) == 3,
		     "La lista debe contener 3 elementos");

	struct pokemon *elemento_eliminado = NULL;
	pa2m_afirmar(lista_quitar_elemento(lista, 2,
					   (void **)&elemento_eliminado),
		     "Eliminar Charmander de la lista");
	pa2m_afirmar(elemento_eliminado != NULL,
		     "El elemento eliminado no es nulo");
	pa2m_afirmar(strcmp(elemento_eliminado->nombre, "Charmander") == 0,
		     "Se eliminó correctamente Charmander");

	pa2m_afirmar(
		lista_cantidad_elementos(lista) == 2,
		"La lista debe contener 2 elementos después de eliminar a Charmander");

	struct pokemon squirtle = { "Squirtle", 'A', 48, 65, 50 };
	pa2m_afirmar(lista_agregar_al_final(lista, &squirtle),
		     "Agregar Squirtle al final de la lista");

	pa2m_afirmar(
		lista_cantidad_elementos(lista) == 3,
		"La lista debe contener 3 elementos después de agregar a Squirtle");
...
int main()
{
	...
	pa2m_nuevo_grupo("Pruebas Adicionales");
	SimularPruebaDeAgregadoYEliminacionMixta();
