...
void imprimir_cantidad_total_pokemones(abb_t *abb)
{
...
void imprimir_resultados(abb_t *pokedex, size_t *contador_tipos)
{
	printf("\n");
	printf("Pokémones en la Pokédex:\n");
	abb_iterar_inorden(pokedex, imprimir_pokemon, NULL);
	printf("\n");
	imprimir_cantidad_total_pokemones(pokedex);
...
int porcesar_entrada(char *entrada, int opcion)
{
	...
	} else if ((strcmp(entrada, SALIR)) == 0 || strcmp(entrada, "0") == 0) {
		opcion = OPCION_SALIR;
	} else {
...
struct pokemon *buscar_pokemon_deseado(abb_t *pokedex)
{
	char entrada[30];
	...
	struct pokemon *resultado = abb_obtener(pokedex, &buscado);
	if (resultado != NULL) {
		return resultado;
	} else {
		return NULL;
...
void procesar_pokemon_buscado(abb_t *pokedex, void *ctx)
{
...
void ejecutar_opciones(abb_t *pokedex, size_t *contador_tipos, void *ctx)
{
